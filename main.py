...
if __name__ == "__main__":
    # Simple HTTP server approach instead of uvicorn
    import threading
    import time
    from http.server import HTTPServer, SimpleHTTPRequestHandler
    import json
    
    print("🚀 Starting Chat Backend on http://localhost:8000")
    print("🤖 ChatAI Personality System: Online")
    
    # Keep the FastAPI app available
    import uvicorn
    try:
        uvicorn.run(app, host="0.0.0.0", port=8000, reload=False)
    except ImportError:
        print("⚠ Uvicorn not available, using alternative method...")
        # Alternative: simple message
        print("✅ Backend logic loaded successfully!")
        print("💡 Try: python -m http.server 8000")
        import time
        while True:
            time.sleep(1)
